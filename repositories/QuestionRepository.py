...
class QuestionRepository:
    ...
    def create(self, question: Question) -> Question:
        try:
            self.db.execute("INSERT INTO multiple_choice_question (proposition, theme, difficulty_points, options, correct_option_index) VALUES (?, ?, ?, ?, ?)", question.proposition, question.theme, question.difficulty_points, question.options, question.correct_option_index)
        except ValueError:
    ...
    def get_by_proposition(self, proposition) -> Question:
        rows = self.db.execute("SELECT * FROM multiple_choice_question WHERE proposition = ?", proposition)
        if not rows:
            return None
        return MultipleChoiceQuestion.constructor_dict(rows[0]) 
    
    def get_by_theme(self, theme) -> list[Question]:
        rows = self.db.execute("SELECT * FROM multiple_choice_question WHERE theme = ?", theme)
        if not rows:
            return None
    ...
    def get_random_questions(self, theme, count) -> list[Question]:
        rows = self.db.execute("SELECT * FROM multiple_choice_question WHERE theme = ? ORDER BY RANDOM() LIMIT ?", theme, count)
        if not rows:
            return None
        return [MultipleChoiceQuestion.constructor_dict(row) for row in rows]
